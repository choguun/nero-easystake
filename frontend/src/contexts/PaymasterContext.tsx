...
export const PaymasterContext = createContext<PaymasterContextType | undefined>(
  undefined,
);
...
export const PaymasterProvider: React.FC<ProviderProps> = ({ children }) => {
  ...
  const [selectedPaymasterType, setSelectedPaymasterType] =
    useState<PaymasterModeValue>(PAYMASTER_MODE.PRE_FUND);
  const [supportedTokens, setSupportedTokens] = useState<PaymasterToken[]>([]);
  const [freeGas, setFreeGas] = useState(false);
  const [paymasterData, setPaymasterData] = useState<PaymasterData | null>(
    null,
  );
