...
const ENTRYPOINT_ABI_DEPOSIT_TO = [
...
const StakePage = () => {
  ...
  const getProvider = useCallback(() => {
    ...
    console.warn(
      "[StakePage] EOA Signer or provider not available from useSignature. Using fallback RPC provider for read-only operations.",
    );
    return new ethers.providers.JsonRpcProvider(
      configRpcUrl || TESTNET_RPC_URL,
    );
  ...
  const fetchNativeBalance = useCallback(async () => {
    ...
    if (AAaddress && AAaddress !== "0x" && provider) {
      ...
      console.warn(
        "[StakePage] Native Balance: provider not available for AA address.",
      );
  ...
  const fetchShareBalance = useCallback(async () => {
    ...
    if (AAaddress && AAaddress !== "0x" && provider && STNERO_ADDRESS) {
      ...
      console.warn(
        "[StakePage] Share Balance: provider not available for AA address shares.",
      );
  ...
  useEffect(() => {
    if (isConnected && AAaddress && AAaddress !== "0x") {
      console.log(
        "[StakePage] Connected. AA Address available. Fetching balances...",
      );
      fetchNativeBalance();
      fetchShareBalance();
    }
  ...
  useEffect(() => {
    ...
    const pollStatus = async () => {
      if (!userOpHash || !isPollingStatus || currentAction === "eoa_fund")
        return; // Don't poll for EOA tx here
      try {
        ...
        if (typeof statusResult === "boolean") {
          ...
          if (
            (statusResult as any).mined === true ||
            (statusResult as any).executed === true
          )
            successful = true;
        ...
        if (successful) {
          console.log(
            `[StakePage] ${currentAction} successful for UserOpHash: ${userOpHash}`,
          );
          setTxStatus(
            `${currentAction === "stake" ? "Staking" : currentAction === "redeem" ? "Redemption" : "UserOp"} successful!`,
          );
          ...
          console.log(
            `[StakePage] ${currentAction} failed for UserOpHash: ${userOpHash}`,
          );
          setTxStatus(
            `${currentAction === "stake" ? "Staking" : currentAction === "redeem" ? "Redemption" : "UserOp"} failed.`,
          );
          setIsPollingStatus(false);
          setCurrentAction(null);
        } else {
  ...
  const handleDirectFundAA = async () => {
    if (!eoaSigner) {
      alert(
        "EOA Signer not available. Please ensure your wallet (e.g., MetaMask) is connected.",
      );
      return;
    }
    ...
    setEoaFundingStatus(
      `Funding AA wallet (${ethersUtils.formatEther(EOA_FUNDING_AMOUNT)} NERO) at EntryPoint...`,
    );
    setEoaFundingTxHash(null);

    try {
      const entryPointInterface = new ethers.utils.Interface(
        ENTRYPOINT_ABI_DEPOSIT_TO,
      );
      const data = entryPointInterface.encodeFunctionData("depositTo", [
        AAaddress,
      ]);

      console.log(
        `[StakePage] Sending EOA transaction to EntryPoint (${entryPointAddress}) for AA (${AAaddress}) with value ${ethersUtils.formatEther(EOA_FUNDING_AMOUNT)} NERO`,
      );
      ...
      setEoaFundingStatus(
        `EOA Funding tx submitted: ${tx.hash}. Awaiting confirmation...`,
      );
      ...
      setEoaFundingStatus(
        `EOA Funding failed: ${error.message || "Unknown error"}`,
      );
      setEoaFundingTxHash(null);
    } finally {
  ...
  const handleStake = async () => {
    ...
    setIsPollingStatus(false);
    console.log("[StakePage] Stake: Set isProcessingTx=true. Current state:", {
      stakeAmount,
      AAaddress,
    });
    ...
    try {
      ...
      console.log(
        "[StakePage] Stake: Calling execute with amount:",
        stakeAmount,
      );
      ...
      console.log(
        "[StakePage] Stake: execute() promise resolved. Result:",
        finalStakeResult,
      );

      if (
        finalStakeResult &&
        finalStakeResult.userOpHash &&
        !finalStakeResult.error &&
        finalStakeResult.result !== false
      ) {
        setUserOpHash(finalStakeResult.userOpHash);
        setTxStatus(
          `Stake UserOp submitted: ${finalStakeResult.userOpHash}. Polling for confirmation...`,
        );
        setIsPollingStatus(true);
        console.log(
          "[StakePage] Stake: Polling started for UserOpHash:",
          finalStakeResult.userOpHash,
        );
      } else {
        const errorMessage =
          finalStakeResult?.error || "Stake submission failed or hash missing.";
        setTxStatus(`Staking Failed: ${errorMessage}`);
        console.log(
          "[StakePage] Stake: Execute completed with failure/error, not polling. Result:",
          finalStakeResult,
        );
      ...
      console.log(
        "[StakePage] Stake: Finally block. Set isProcessingTx=false.",
      );
    }
  };
  ...
  const handleRedeem = async () => {
    ...
    setIsPollingStatus(false);
    console.log("[StakePage] Redeem: Set isProcessingTx=true. Current state:", {
      redeemAmount,
      AAaddress,
    });
    let finalResult: UserOperationResultInterface | null = null;
    try {
      const sharesToRedeem = ethersUtils.parseUnits(
        redeemAmount,
        VAULT_DECIMALS,
      );
      console.log(
        "[StakePage] Redeem: Calling execute with shares:",
        redeemAmount,
      );
      ...
      console.log(
        "[StakePage] Redeem: execute() promise resolved. Result:",
        finalResult,
      );

      if (
        finalResult &&
        finalResult.userOpHash &&
        !finalResult.error &&
        finalResult.result !== false
      ) {
        setUserOpHash(finalResult.userOpHash);
        setTxStatus(
          `Redeem UserOp submitted: ${finalResult.userOpHash}. Polling for confirmation...`,
        );
        setIsPollingStatus(true);
        console.log(
          "[StakePage] Redeem: Polling started for UserOpHash:",
          finalResult.userOpHash,
        );
      } else {
        const errorMessage =
          finalResult?.error || "Redeem submission failed or hash missing.";
        setTxStatus(`Redemption Failed: ${errorMessage}`);
        console.log(
          "[StakePage] Redeem: Execute completed with failure/error, not polling. Result:",
          finalResult,
        );
      ...
      setIsProcessingTx(false);
      console.log(
        "[StakePage] Redeem: Finally block. Set isProcessingTx=false.",
      );
    }
  };
  ...
  const getButtonText = (actionType: "stake" | "redeem") => {
    if (currentAction === actionType) {
      // Only show specific status if it's for this button's action
      if (
        isProcessingTx &&
        (actionType === "stake" || actionType === "redeem")
      ) {
      ...
      if (
        ...
        return txStatus.startsWith("Confirming") ||
          txStatus.startsWith("UserOp submitted") ||
          txStatus.startsWith("Status: Still processing")
          ? txStatus || "Polling..."
          : actionType === "stake"
            ? "Stake NERO"
            : "Redeem Shares";
  ...
  return (
    <div
      className="container mx-auto p-4 pt-10 min-h-screen bg-gray-900 text-white"
      data-oid="zgl71-w"
    >
      <h1
        ...
        data-oid="b012dd-"
      >
        AA Stake & Redeem NERO
      </h1>

      {!isConnected && (
        <div className="flex justify-center" data-oid="_owr3_7">
          <ConnectButton data-oid="e9.3xr-" />
        </div>
      )}

      {isConnected && AAaddress && AAaddress !== "0x" ? (
        <div
          className="bg-gray-800 shadow-xl rounded-lg p-6 mb-8 max-w-md mx-auto"
          data-oid="slyiao2"
        >
          <p className="text-lg mb-1 text-gray-300" data-oid="ue.gak5">
            AA Wallet:{" "}
            <span
              className="font-mono text-sm text-purple-300 break-all"
              data-oid="lhculkd"
            >
              {AAaddress}
            </span>
          </p>
          <p className="text-lg mb-1 text-gray-300" data-oid="53tskih">
            NERO Balance (AA Wallet):{" "}
          ...
          <p className="text-lg mb-2 text-gray-300" data-oid="qorvfi5">
            Vault Shares (stNERO):{" "}
          ...
          {eoaSigner &&
            ...
            AAaddress !== "0x" && (
              <div
                className="mt-2 pt-3 border-t border-gray-700"
                data-oid="oiu5588"
              >
                {/* <button 
            ...
            {isProcessingEoaFunding ? 'Funding AA via EOA...' : `Fund AA at EntryPoint (${ethersUtils.formatEther(EOA_FUNDING_AMOUNT)} NERO via EOA)`}
                ...
                {eoaFundingStatus && (
                  <p
                    className={`text-xs mt-2 text-center ${eoaFundingStatus.includes("failed") || eoaFundingStatus.includes("Error") ? "text-red-400" : "text-green-400"}`}
                    data-oid="1g:j.gu"
                  >
                    EOA Funding Status: {eoaFundingStatus}
                    {eoaFundingTxHash && (
                      <span data-oid="zidml_g">
                        {" "}
                        (Tx:{" "}
                        <a
                          href={`https://testnet.neroscan.io/tx/${eoaFundingTxHash}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="underline hover:text-purple-300"
                          data-oid="dzzgt2t"
                        >
                          {eoaFundingTxHash.substring(0, 6)}...
                          {eoaFundingTxHash.substring(
                            eoaFundingTxHash.length - 4,
                          )}
                        </a>
                        )
                      </span>
        ...
        <p
          className="text-center text-xl text-gray-400 mb-8"
          data-oid="fz0d.fb"
        >
          Please connect your wallet (e.g., MetaMask for EOA, then link/create
          AA).
        </p>
      )}
      ...
      {isConnected && AAaddress && AAaddress !== "0x" && (
        <div
          className="bg-gray-800 shadow-xl rounded-lg p-6 mb-4 max-w-md mx-auto"
          data-oid="5brx2z."
        >
          <h2
            className="text-2xl font-semibold mb-3 text-center text-purple-300"
            data-oid="gpocc0f"
          >
            Stake NERO
          </h2>
          <input
            ...
            placeholder="Amount of NERO to stake"
            disabled={
              isProcessingTx || isPollingStatus || isProcessingEoaFunding
            }
          ...
          <button
            ...
            disabled={
              ...
              nativeBalance === "Error" ||
              parseFloat(nativeBalance) < parseFloat(stakeAmount)
            }
            className="w-full px-6 py-3 font-semibold text-white bg-green-600 rounded-md hover:bg-green-700 disabled:opacity-50 transition-colors"
            data-oid="vmlsx-g"
          >
            {getButtonText("stake")}
          </button>
      ...
      {isConnected && AAaddress && AAaddress !== "0x" && (
        <div
          className="bg-gray-800 shadow-xl rounded-lg p-6 max-w-md mx-auto"
          data-oid="jvpc1zm"
        >
          <h2
            className="text-2xl font-semibold mb-3 text-center text-purple-300"
          ...
          <button
            ...
            disabled={
              ...
              shareBalance === "Error" ||
              parseFloat(shareBalance) < parseFloat(redeemAmount)
            }
            className="w-full px-6 py-3 font-semibold text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:opacity-50 transition-colors"
            data-oid="00elg38"
          >
      ...
      {isConnected && AAaddress && AAaddress !== "0x" && (
        <div
          className="bg-gray-800 shadow-xl rounded-lg p-6 max-w-md mx-auto mt-6"
          data-oid="py4:zn1"
        >
          <h2
            className="text-2xl font-semibold mb-3 text-center text-purple-300"
          ...
          {sendStatus && (
            <p
              className={`mt-2 text-center text-sm ${sendStatus.includes("error") || sendStatus.includes("failed") ? "text-red-400" : "text-blue-400"}`}
              data-oid=".:sywil"
            >
              {sendStatus}
            </p>
      ...
      {(userOpHash ||
        (currentAction === "stake" &&
          txStatus &&
          !isPollingStatus &&
          !isProcessingTx)) && ( // Show status for UserOps
        <div
          className="mt-6 p-4 bg-gray-800 rounded-lg shadow-xl max-w-md mx-auto text-center"
          data-oid="1-03f3b"
        >
          <p
            className="text-md font-semibold text-purple-300 mb-1"
            data-oid="nf-o03f"
          >
          ...
          {userOpHash && (
            <p
              className="text-xs text-gray-400 mb-1 break-all"
              data-oid="b:cmro6"
            >
              UserOp Hash:{" "}
              <span className="font-mono text-purple-300" data-oid="ofk.32s">
                {userOpHash}
              </span>
            </p>
          )}
          <p
            className={`text-sm font-medium ${txStatus.includes("successful") ? "text-green-400" : txStatus.includes("failed") || txStatus.includes("Error") ? "text-red-400" : "text-blue-400"}`}
            data-oid="6he5faa"
          >
