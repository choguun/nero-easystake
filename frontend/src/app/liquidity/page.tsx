...
import { UserOperationResultInterface, UserOperation } from "@/types";
import {
...
export default function LiquidityPage() {
  ...
  const [pairToken0Address, setPairToken0Address] = useState<string | null>(
    null,
  );
  const [poolReserveA, setPoolReserveA] = useState<BigNumber>(
    BigNumber.from(0),
  ); // Corresponds to tokenAInfo
  const [poolReserveB, setPoolReserveB] = useState<BigNumber>(
    BigNumber.from(0),
  ); // Corresponds to tokenBInfo
  const [poolLpTotalSupply, setPoolLpTotalSupply] = useState<BigNumber>(
    BigNumber.from(0),
  );
  ...
  const [estimatedTokenAReceived, setEstimatedTokenAReceived] =
    useState<string>("");
  const [estimatedTokenBReceived, setEstimatedTokenBReceived] =
    useState<string>("");
  ...
  const fetchTokenMetadataAndBalance = useCallback(
    async (
      ...
      console.log(
        `[LiquidityPage] fetchTokenMetadataAndBalance called for ${tokenAddress}${isNative ? " (Native)" : ""}. AAaddress: ${AAaddress}, Provider exists: ${!!provider}`,
      );
      try {
        if (isNative) {
          console.log(
            `[LiquidityPage] Fetching native balance for ${AAaddress}`,
          );
          ...
          console.log(
            `[LiquidityPage] Fetched native token info for ${tokenAddress}:`,
            tokenInfo,
          );
          return tokenInfo;
        }
        console.log(
          `[LiquidityPage] Creating contract instance for ERC20 token: ${tokenAddress}`,
        );
        const contract = new ethers.Contract(tokenAddress, ERC20_ABI, provider);
        console.log(
          `[LiquidityPage] Fetching name, symbol, decimals, balance for ${tokenAddress} for account ${AAaddress}`,
        );
        ...
        console.log(
          `[LiquidityPage] Fetched ERC20 token info for ${tokenAddress}:`,
          tokenInfo,
        );
        return tokenInfo;
      } catch (error) {
        console.error(
          `[LiquidityPage] Error in fetchTokenMetadataAndBalance for ${tokenAddress}:`,
          error,
        );
        toast({
          title: "Data Fetch Error",
          description: `Could not load details for token ${tokenAddress}`,
          variant: "destructive",
        });
        return null;
      }
  ...
  const fetchAllowance = useCallback(
    async (
      ...
      if (!provider) {
        console.log(
          `[LiquidityPage] fetchAllowance: Provider not available for ${tokenAddress}.`,
        );
        return "0";
      }
      console.log(
        `[LiquidityPage] fetchAllowance called for token: ${tokenAddress}, owner: ${owner}, spender: ${spender}, decimals: ${decimals}`,
      );
      try {
        const contract = new ethers.Contract(tokenAddress, ERC20_ABI, provider);
        console.log(
          `[LiquidityPage] Fetching allowance for ${tokenAddress} from ${owner} to ${spender}`,
        );
        const allowance = await contract.allowance(owner, spender);
        const formattedAllowance = ethersUtils.formatUnits(allowance, decimals);
        console.log(
          `[LiquidityPage] Fetched allowance for ${tokenAddress}: ${formattedAllowance}`,
        );
        return formattedAllowance;
      } catch (error) {
        console.error(
          `[LiquidityPage] Error in fetchAllowance for ${tokenAddress}:`,
          error,
        );
        toast({
          title: "Allowance Fetch Error",
          description: `Could not load allowance for ${tokenAddress}`,
          variant: "destructive",
        });
        return "0";
      }
  ...
  const refetchAllData = useCallback(async () => {
    ...
    const lpDataPromise = fetchTokenMetadataAndBalance(
      WNERO_STNERO_PAIR_ADDRESS,
    );

    const [wneroData, stneroData, lpData] = await Promise.all([
      wneroDataPromise,
      stneroDataPromise,
      lpDataPromise,
    ]);
    ...
    if (wneroData) {
      wneroData.allowance = await fetchAllowance(
        WNERO_ADDRESS,
        AAaddress,
        UNISWAP_ROUTER_ADDRESS,
        wneroData.decimals,
    ...
    if (stneroData) {
      stneroData.allowance = await fetchAllowance(
        STNERO_ADDRESS,
        AAaddress,
        UNISWAP_ROUTER_ADDRESS,
        stneroData.decimals,
    ...
    if (lpData) {
      lpData.allowance = await fetchAllowance(
        WNERO_STNERO_PAIR_ADDRESS,
        AAaddress,
        UNISWAP_ROUTER_ADDRESS,
        lpData.decimals,
  ...
  useEffect(() => {
    console.log("[LiquidityPage] useEffect triggered. Dependencies:", {
      isConnected,
      AAaddress,
    ...
    const init = async () => {
      ...
      if (isConnected && AAaddress && provider) {
        setIsLoading(true);
        console.log(
          "[LiquidityPage] Starting initial token data fetching process...",
        );
        ...
        console.log(
          "[LiquidityPage] Awaiting all token metadata and balance promises...",
        );
        const [wneroData, stneroData, lpData] = await Promise.all([
          wneroDataPromise,
          stneroDataPromise,
        ...
        console.log(
          "[LiquidityPage] All token metadata and balance promises resolved:",
          { wneroData, stneroData, lpData },
        );

        if (wneroData) {
          console.log(
            "[LiquidityPage] WNERO data received, fetching allowance...",
          );
          wneroData.allowance = await fetchAllowance(
            WNERO_ADDRESS,
            AAaddress,
          ...
          setTokenAInfo(wneroData);
          console.log(
            "[LiquidityPage] WNERO data and allowance set to state:",
            wneroData,
          );
        } else {
          console.warn(
            "[LiquidityPage] No WNERO data received from fetchTokenMetadataAndBalance.",
          );
          setTokenAInfo(null); // Explicitly set to null if no data
        }
        if (stneroData) {
          console.log(
            "[LiquidityPage] STNERO data received, fetching allowance...",
          );
          stneroData.allowance = await fetchAllowance(
            STNERO_ADDRESS,
            AAaddress,
          ...
          setTokenBInfo(stneroData);
          console.log(
            "[LiquidityPage] STNERO data and allowance set to state:",
            stneroData,
          );
        } else {
          console.warn(
            "[LiquidityPage] No STNERO data received from fetchTokenMetadataAndBalance.",
          );
        ...
        if (lpData) {
          console.log(
            "[LiquidityPage] LP token data received, fetching allowance...",
          );
          lpData.allowance = await fetchAllowance(
            WNERO_STNERO_PAIR_ADDRESS,
            AAaddress,
          ...
          setLpTokenInfo(lpData);
          console.log(
            "[LiquidityPage] LP token data and allowance set to state:",
            lpData,
          );

          if (provider) {
            console.log(
              "[LiquidityPage] Provider exists, fetching LP token total supply for pair:",
              WNERO_STNERO_PAIR_ADDRESS,
            );
            try {
              ...
              console.log(
                "[LiquidityPage] LP Token Total Supply fetched successfully:",
                totalSupply.toString(),
              );
            } catch (error) {
              console.error(
                "[LiquidityPage] Error fetching LP token total supply:",
                error,
              );
              toast({
                title: "LP Data Error",
                description: "Could not load LP token total supply.",
                variant: "destructive",
              });
            ...
            console.warn(
              "[LiquidityPage] Provider not available for fetching LP total supply when LP data was present.",
            );
          }
        } else {
          console.warn(
            "[LiquidityPage] No LP token data received. Resetting LP total supply and info.",
          );
        ...
        // Fetch pair reserves
        console.log(
          "[LiquidityPage] Checking conditions for fetching pair reserves:",
          {
        ...
        if (WNERO_STNERO_PAIR_ADDRESS && provider && wneroData && stneroData) {
          // Ensure wneroData & stneroData are truthy
          try {
            console.log(
              "[LiquidityPage] Conditions met. Attempting to fetch reserves from pair:",
              WNERO_STNERO_PAIR_ADDRESS,
            );
            ...
            console.log("[LiquidityPage] Raw reserves fetched:", {
            ...
            if (t0Address.toLowerCase() === WNERO_ADDRESS.toLowerCase()) {
              ...
              console.log(
                "[LiquidityPage] Pool Reserves ALIGNED (WNERO/Token0, STNERO/Token1):",
                finalReserveA.toString(),
                finalReserveB.toString(),
              );
            } else if (
              t0Address.toLowerCase() === STNERO_ADDRESS.toLowerCase()
            ) {
              finalReserveA = rawReserves[1]; // WNERO is token1 in pair
              finalReserveB = rawReserves[0]; // STNERO is token0 in pair
              console.log(
                "[LiquidityPage] Pool Reserves ALIGNED (STNERO/Token0, WNERO/Token1):",
                finalReserveA.toString(),
                finalReserveB.toString(),
              );
            } else {
              ...
              console.warn(
                "[LiquidityPage] Pair tokens don't match WNERO/STNERO. WNERO:",
                WNERO_ADDRESS,
                "STNERO:",
                STNERO_ADDRESS,
                "Pair t0:",
            ...
            console.error(
              "[LiquidityPage] Error fetching pair reserves:",
              error,
            );
            toast({
              title: "Pool Data Error",
              description: "Could not load pool reserves.",
              variant: "destructive",
            });
          ...
          console.warn(
            "[LiquidityPage] SKIPPED reserve fetching due to unmet conditions (missing provider, pair address, or token data). Resetting reserves.",
          );
          setPoolReserveA(BigNumber.from(0));
          setPoolReserveB(BigNumber.from(0));
        }
        setIsLoading(false);
        console.log(
          "[LiquidityPage] Initial data fetching process complete. isLoading set to false.",
        );
      } else {
        console.warn(
          "[LiquidityPage] Conditions not met for initial data fetch in useEffect:",
          { isConnected, AAaddress, providerExists: !!provider },
        );
    ...
    init().catch((err) => {
      console.error(
        "[LiquidityPage] Unhandled error in init function promise chain:",
        err,
      );
      ...
      toast({
        ...
        description:
          "An unexpected error occurred while initializing the page.",
        variant: "destructive",
      });
    });
  ...
  const handleApprove = useCallback(
    async (tokenInfo: TokenInfo, amount: string) => {
      ...
      if (!amountToApprove || amountToApprove.isZero()) {
        toast({
          ...
          description: "Approval amount must be greater than zero.",
          variant: "destructive",
        });
        setIsProcessing(false);
        return;
      ...
      try {
        ...
        if (result.userOpHash && !result.error) {
          ...
          setTxStatusMessage(
            `Approval submitted for ${tokenInfo.symbol}. Waiting for confirmation...`,
          );
  ...
  const handleAddLiquidity = async () => {
    if (!tokenAInfo || !tokenBInfo || !UNISWAP_ROUTER_ADDRESS || !AAaddress) {
      toast({
        title: "Missing Data",
        description: "Token information or router address is missing.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (
      !amountADesired ||
      amountADesired.isZero() ||
      !amountBDesired ||
      amountBDesired.isZero()
    ) {
      toast({
    ...
    const slippageMultiplier = BigNumber.from(
      Math.floor((1 - DEFAULT_SLIPPAGE_TOLERANCE / 100) * 10000),
    ); // e.g., 0.5% slippage -> 9950
    ...
    try {
      ...
      if (result.userOpHash && !result.error) {
        setUserOpHash(result.userOpHash);
        setTxStatusMessage(
          "Add liquidity transaction submitted. Waiting for confirmation...",
        );
  ...
  const handleApproveLpTokens = useCallback(
    async (lpAmountToRemove: BigNumber) => {
      if (!lpTokenInfo || !UNISWAP_ROUTER_ADDRESS) {
        toast({
          title: "Missing Info",
          description:
            "LP token information or router address is not available.",
          variant: "default",
        });
        return false;
      }
      if (lpAmountToRemove.isZero()) {
        toast({
          title: "Invalid Amount",
          description: "LP amount to approve cannot be zero.",
          variant: "default",
        });
        return false;
      }
      ...
      try {
        ...
        if (result.userOpHash && !result.error) {
          setUserOpHash(result.userOpHash);
          setTxStatusMessage(
            `Approval submitted for ${lpTokenInfo.symbol}. Waiting for confirmation...`,
          );
          ...
          toast({
            title: "Approval Sent",
            description: `User operation ${result.userOpHash} submitted.`,
          });
          return true; // Indicate approval was sent
        } else {
          throw new Error(
            result.error || "LP Token approval failed to submit.",
          );
        }
      } catch (error: any) {
        console.error("[LiquidityPage] LP Approval Error:", error);
        toast({
          title: "LP Approval Error",
          description: error.message || "Could not submit LP approval.",
          variant: "destructive",
        });
        setIsProcessing(false);
        return false;
  ...
  const handleRemoveLiquidity = async () => {
    if (
      ...
      toast({
        ...
        description:
          "Please connect your wallet and ensure all token data is loaded.",
        variant: "default",
      });
      return;
    }
    ...
    try {
      ...
      const calculatedLpAmountToRemove = parseUnitsSafe(
        removeLpAmount,
        lpDecimals,
      );

      if (
        !calculatedLpAmountToRemove ||
        calculatedLpAmountToRemove.isZero() ||
        poolLpTotalSupply.isZero()
      ) {
        toast({
          title: "Invalid Input",
      ...
      const currentLpAllowance = parseUnitsSafe(
        lpTokenInfo.allowance || "0",
        lpDecimals,
      );
      if (currentLpAllowance.lt(calculatedLpAmountToRemove)) {
        toast({
          title: "Approval Required",
          description: `You need to approve ${lpTokenInfo.symbol} for removal.`,
          variant: "default",
        });
        const approvalSent = await handleApproveLpTokens(
          calculatedLpAmountToRemove,
        );
        if (!approvalSent) {
          // If approval wasn't even submitted (e.g. user rejected, or immediate error)
        ...
        setTxStatusMessage(
          "LP Token approval submitted. Please wait for confirmation and try removing again.",
        );
        ...
        setLpTokenInfo((prev) =>
          prev ? { ...prev, allowance: newAllowance } : null,
        );
        if (
          parseUnitsSafe(newAllowance, lpTokenInfo.decimals).lt(
            calculatedLpAmountToRemove,
          )
        ) {
          toast({
            ...
            description:
              "Approval still pending or insufficient. Please wait and try again.",
            variant: "default",
          });
          setIsProcessing(false);
          return;
      ...
      const expectedAmountA = liquidityToRemove
        .mul(poolReserveA)
        .div(poolLpTotalSupply);
      const expectedAmountB = liquidityToRemove
        .mul(poolReserveB)
        .div(poolLpTotalSupply);
      ...
      const amountETHMin = expectedAmountA
        .mul(slippageMultiplier)
        .div(slippageDivisor);
      const amountTokenMin = expectedAmountB
        .mul(slippageMultiplier)
        .div(slippageDivisor);
      ...
      if (result.userOpHash && !result.error) {
        setUserOpHash(result.userOpHash);
        setTxStatusMessage(
          `Remove liquidity submitted: ${result.userOpHash}. Waiting for confirmation...`,
        );
        toast({
      ...
      toast({
        ...
        description:
          error.message || "Could not submit remove liquidity transaction.",
        variant: "destructive",
      });
    } finally {
      handleCancel();
  ...
  const handleAddAmountAChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (/^\d*\.?\d*$/.test(value)) {
      ...
      console.log(
        "[handleAddAmountAChange] Attempting calculation. Input value:",
        value,
      );
  ...
  if (sigLoading) {
    return (
      <div
        ...
        data-oid="f_z4:3z"
      >
        <Loader2 className="h-8 w-8 animate-spin" data-oid="tn.ou8u" />
      </div>
    );
  }
  ...
  return (
    <div
      ...
      data-oid="xw_42we"
    >
      <Card className="shadow-lg w-full max-w-xl" data-oid="n93rn99">
        <CardHeader data-oid=":piu_l.">
          <CardTitle
            className="text-2xl flex items-center gap-2"
            data-oid="0iky2_h"
          >
            <Droplets className="h-6 w-6 text-primary" data-oid="fs2ae1a" />
            Manage Liquidity
          </CardTitle>
          <CardDescription data-oid=":6.8bd7">
            Add or remove liquidity. Earn fees by providing liquidity.
          </CardDescription>
        </CardHeader>
        <CardContent data-oid="d604mex">
          <Tabs
            value={activeTab}
            ...
            data-oid="-cm67lz"
          >
            <TabsList className="grid w-full grid-cols-2" data-oid="7.v0j6f">
              <TabsTrigger
                value="add"
                className="flex items-center gap-1"
                data-oid="c3wkb7."
              >
                <PlusCircle className="h-4 w-4" data-oid=":qtup21" /> Add
                Liquidity
              </TabsTrigger>
              <TabsTrigger
                value="remove"
                ...
                data-oid="mtx-wur"
              >
                <MinusCircle className="h-4 w-4" data-oid="jnhltqy" /> Remove
                Liquidity
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent
              ...
              data-oid="gl6j_ke"
            >
              <div className="space-y-2" data-oid="vi3p6nb">
                <div
                  className="flex justify-between items-baseline"
                  data-oid="73gb12g"
                >
                  <Label htmlFor="add-amount-a" data-oid="on7-t6c">
                    {tokenAInfo?.symbol || "Token A"} Amount
                  </Label>
                  <span
                  ...
                  >
                    ...
                    {isLoading
                      ? "Loading..."
                      : parseFloat(tokenAInfo?.balance || "0").toFixed(4)}
                  </span>
                </div>
                <Input
                  id="add-amount-a"
                ...
                {!isTokenAApproved &&
                  parseFloat(addAmountA) > 0 &&
                  tokenAInfo && (
                    <Button
                      onClick={() => handleApprove(tokenAInfo, addAmountA)}
              ...
              <div
                ...
                data-oid="f1sj-u8"
              >
                <PlusCircle className="h-5 w-5" data-oid="txzssz_" />
              </div>

              <div className="space-y-2" data-oid="-8j5.x1">
                <div
                  className="flex justify-between items-baseline"
                  data-oid="9fv-u55"
                >
                  <Label htmlFor="add-amount-b" data-oid="a8qp4f3">
                    {tokenBInfo?.symbol || "Token B"} Amount
                  </Label>
                  <span
                    className="text-sm text-muted-foreground"
                    data-oid="bjb48o1"
                  >
                    ...
                    {isLoading
                      ? "Loading..."
                      : parseFloat(tokenBInfo?.balance || "0").toFixed(4)}
                  </span>
                </div>
                <Input
                  id="add-amount-b"
                ...
                {!isTokenBApproved &&
                  parseFloat(addAmountB) > 0 &&
                  tokenBInfo && (
                    <Button
                      onClick={() => handleApprove(tokenBInfo, addAmountB)}
                      disabled={isProcessing}
                      className="w-full mt-1"
                      data-oid="m0h2j7u"
                    >
              ...
              <Button
                ...
                disabled={
                  ...
                  !isTokenAApproved ||
                  !isTokenBApproved ||
                  !parseFloat(addAmountA) ||
                  !parseFloat(addAmountB)
                }
                data-oid=":1qc1fh"
              >
                {isProcessing
                  ? txStatusMessage.includes("submitted")
            ...
            <TabsContent
              ...
              data-oid="q:_7hn9"
            >
              <div className="space-y-2" data-oid="kajjzlk">
                <div
                  className="flex justify-between items-baseline"
                  data-oid="_t3dwnq"
                >
                  <Label htmlFor="remove-lp-percent" data-oid="iz9v2j1">
                    Amount to Remove ({removeLpPercent[0]}%)
                  </Label>
                  <span
                    className="text-sm text-muted-foreground"
                    data-oid="hedh7-i"
                ...
                <div className="flex items-center gap-2" data-oid="k:bqqo2">
                  <Slider
              ...
              <Button
                ...
                disabled={
                  isProcessing || isLoading || parseFloat(removeLpAmount) <= 0
                }
                data-oid="rlbkp6-"
              >
        ...
        {txStatusMessage && (
          <CardFooter
            className="flex flex-col items-center justify-center pt-4 border-t"
            data-oid="nblbkq-"
          >
            <p
              className={`text-sm ${txStatusMessage.includes("failed") || txStatusMessage.includes("Error") ? "text-red-500" : "text-green-500"}`}
              data-oid="q.oiw95"
            >
              {txStatusMessage}
            </p>
