...
export const SignatureProvider: React.FC<ProviderProps> = ({ children }) => {
  ...
  const [simpleAccountInstance, setSimpleAccountInstance] = useState<
    SimpleAccount | undefined
  >(undefined);
  const [sessionEoaAddress, setSessionEoaAddress] = useState<
    `0x${string}` | null
  >(null);
  ...
  const resetSignature = useCallback(() => {
    ...
    console.log(
      "[SignatureContext] resetSignature called. EOA from state:",
      sessionEoaAddress,
    );
    setLoading(false);
    setAAaddress("0x");
    ...
    if (sessionEoaAddress) {
      ...
      console.log(
        "[SignatureContext] AA session cleared from localStorage for EOA:",
        sessionEoaAddress,
      );
    }
  ...
  const connectAA = useCallback(
    async (pm?: "token" | "verifying" | "legacy-token") => {
      if (!signer || !eoaAddress) {
        console.error(
          "[SignatureContext] connectAA: Signer or EOA address is not available.",
        );
        return false;
      }
      console.log(
        "[SignatureContext] connectAA: Initializing AA wallet for EOA:",
        eoaAddress,
      );
      setLoading(true);
      try {
        ...
        const derivedAAaddress =
          (await simpleAccount.getSender()) as `0x${string}`;
        ...
        console.error(
          "[SignatureContext] Error during AA wallet connection:",
          e,
        );
        // Clean up on failure
        if (eoaAddress) {
  ...
  useEffect(() => {
    // If a session is already active in the state, we still need to check if it matches the current EOA
    // This ensures proper restoration when switching between tabs
    if (AAaddress && AAaddress !== "0x" && sessionEoaAddress === eoaAddress) {
      if (loading) setLoading(false); // Ensure loading is false if we bail early.
      return;
    }

    if (isEoaWalletConnected && signer && eoaAddress && chain) {
      const storedAaAddress = localStorage.getItem(
        `${AA_ADDRESS_KEY_PREFIX}${eoaAddress}`,
      ) as `0x${string}` | null;
      // If we find an address in storage, we try to restore the session.
      if (storedAaAddress && storedAaAddress !== "0x") {
        console.log(
          "[SignatureContext] Found stored AA session for EOA:",
          eoaAddress,
          "with AA:",
          storedAaAddress,
        );
        ...
        SimpleAccount.init(signer, rpcUrl, {
          ...
          .then(async (saInstance) => {
            ...
            if (derivedSender === storedAaAddress) {
              ...
              console.warn(
                "[SignatureContext] Stored AA address mismatch. Clearing stored session.",
              );
              resetSignature();
            }
          })
          .catch((err) => {
            console.error(
              "[SignatureContext] Error re-initializing SimpleAccount for session restore:",
              err,
            );
            setLoading(false);
            resetSignature();
