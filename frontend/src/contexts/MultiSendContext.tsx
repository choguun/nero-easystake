...
export const MultiSendContext = createContext<
  MultiSendContextProps | undefined
>(undefined);
...
export const MultiSendProvider: React.FC<ProviderProps> = ({ children }) => {
  ...
  const [activeTokenModalIndex, setActiveTokenModalIndex] = useState<
    number | null
  >(null);
  const [totalAmountByToken, setTotalAmountByToken] = useState<
  ...
  const validateForm = useCallback(() => {
    return recipients.every((recipient) => {
      const isAddressValid =
        recipient.address.length > 0 && isValidAddress(recipient.address);
      const isAmountValid =
