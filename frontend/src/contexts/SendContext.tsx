...
export const SendProvider: React.FC<ProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    if (lastSentToken) {
      setSelectedToken({
        ...lastSentToken,
        balance: formatAndRoundBalance(
          lastSentToken.balance,
          lastSentToken.decimals,
        ),
      });
      setBalance(
        formatAndRoundBalance(lastSentToken.balance, lastSentToken.decimals),
      );
